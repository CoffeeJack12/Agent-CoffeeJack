...
import { slimSecurityForRemote } from "./security/evidence.mjs";
...
export function sanitizeForRemote(text = "", { maxChars = 24000 } = {}) {
  ...
  const slimmed = slimSecurityToolText(out);
  if (slimmed) {
    out = slimmed;
    redacted += 1;
  }
...
function slimSecurityToolText(text) {
  const raw = String(text || "").trim();
  if (!raw.startsWith("{") || !raw.includes('"observed"')) return null;
  try {
    const obj = JSON.parse(raw);
    const payload = obj.tool ? obj : obj.observed && obj._attribution ? obj : null;
    if (!payload?.tool || !String(payload.tool).startsWith("security_"))
      return null;
    const { _attribution, ...rest } = payload;
    const slim = slimSecurityForRemote(rest);
    if (_attribution) slim._attribution = _attribution;
